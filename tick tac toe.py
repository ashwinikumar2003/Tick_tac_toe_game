...
# Fonts
END_FONT = pygame.font.SysFont('verdana', 50)
CHAR_FONT = pygame.font.SysFont('comicsansms', 80)
        ...
        for j in range(len(game_array[i])):
            ...
            if dis < WIDTH // ROWS // 2 and can_play:
                if x_turn:  # If it's X's turn
                    char_surface = CHAR_FONT.render("X", 1, RED)
                    images.append((x, y, char_surface))
                    x_turn = False
                    o_turn = True
                    game_array[i][j] = (x, y, 'x', False)

                elif o_turn:  # If it's O's turn
                    char_surface = CHAR_FONT.render("O", 1, BLUE)
                    images.append((x, y, char_surface))
                    x_turn = True
                    o_turn = False
